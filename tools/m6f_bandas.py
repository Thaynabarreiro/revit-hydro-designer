# -*- coding: utf-8 -*-
"""M6f - Rede ortogonal com agrupamento por faixa.
...
t = Transaction(doc, "M6f - corrigir cota do reservatorio")
t.Start()
if res_inst is not None:
    try:
        ...
        print("aviso ao reposicionar reservatorio: " + str(e)[:60])
t.Commit()

if res_inst is not None:
    p_res = res_inst.Location.Point
else:
...
# no da faixa: Y medio das pecas
for fx in faixas:
    fx["y"] = sum([p["org"].Y for p in fx["pecas"]]) / len(fx["pecas"])
    fx["peso"] = sum([p["peso"] for p in fx["pecas"]])

# A espinha nao pode ir e voltar sobre a mesma linha. Divide-se em dois ramos
# a partir da coluna, cada um monotonico em Y.
...
print("pecas: {0} | faixas: {1} | peso: {2:.2f}".format(
    len(pecas), len(faixas), peso_total))
for i, fx in enumerate(faixas):
    print("  faixa {0}: {1} peca(s), y = {2:.0f} mm, peso {3:.2f}".format(
        i + 1, len(fx["pecas"]), mm(fx["y"]), fx["peso"]))

# =========================================================== T1: tubos
t = Transaction(doc, "M6f - rede por faixas")
t.Start()
...
for i, fx in enumerate(faixas):
    ...
    fx["p_no"] = p_no
    fx["esp"] = tubo(p_ant, p_no, diametro(C * math.sqrt(max(peso_rest, 0.01))),
                     "espinha " + str(i + 1))

    # ramal da faixa: percorre em X atendendo as pecas, da mais proxima a mais longe
    ...
    for j, pc in enumerate(membros):
        ...
        pc["ramal"] = tubo(p_cursor, p_alvo,
                           ...
                           "ramal {0}.{1}".format(i + 1, j + 1))
        pc["p_ramal"] = p_alvo
        pc["desc"] = tubo(p_alvo, pc["org"], diametro(C * math.sqrt(pc["peso"])),
                          "descida {0}.{1}".format(i + 1, j + 1))
        p_cursor = p_alvo
        peso_ramal -= pc["peso"]
...
# ================================================== T2: ligar as pecas
t = Transaction(doc, "M6f - ligar pecas")
t.Start()
ligadas = 0
for fx in faixas:
    for pc in fx["pecas"]:
...
# ============================================== T3: tes e joelhos
t = Transaction(doc, "M6f - conexoes")
t.Start()
fit_ok, fit_falha = 0, []
...
def liga(a, b, ponto, rot, terceiro=None):
    ...
    try:
        ...
        fit_falha.append((rot, str(e)[:55]))


# pe da coluna
if coluna is not None and faixas:
    liga(coluna, faixas[0]["esp"], no0, "pe da coluna")

for i, fx in enumerate(faixas):
    membros = sorted(fx["pecas"], key=lambda p: abs(p["org"].X - x_esp))
    prox = faixas[i + 1]["esp"] if i + 1 < len(faixas) else None
    primeiro_ramal = membros[0].get("ramal") if membros else None
    saida = primeiro_ramal if primeiro_ramal is not None else (
        membros[0].get("desc") if membros else None)

    if prox is not None:
        liga(fx["esp"], prox, fx["p_no"], "te faixa " + str(i + 1), terceiro=saida)
    else:
        liga(fx["esp"], saida, fx["p_no"], "joelho faixa final")

    # ao longo do ramal da faixa
    for j, pc in enumerate(membros):
        prox_ramal = membros[j + 1].get("ramal") if j + 1 < len(membros) else None
        if pc.get("ramal") is None:
            continue
        if prox_ramal is not None:
            liga(pc["ramal"], prox_ramal, pc["p_ramal"],
                 "te {0}.{1}".format(i + 1, j + 1), terceiro=pc.get("desc"))
        else:
            liga(pc["ramal"], pc.get("desc"), pc["p_ramal"],
                 "joelho {0}.{1}".format(i + 1, j + 1))
